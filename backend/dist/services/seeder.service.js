...
function calcResolutionMinutes(startStr, endStr, closedStr, resolvedStr) {
...
async function seedDatabase() {
    logger_1.logger.info('🔄 System Seed Request Triggered.');
    ...
    logger_1.logger.info(`✅ Seeded ${events.length} events.`);
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        try {
            ...
            const mappedPriority = priority === 'High' ? 'HIGH' : 'LOW';
            const mappedType = eventType === 'planned' ? 'PLANNED' : 'UNPLANNED';
            ...
            const resolutionMinutes = calcResolutionMinutes(startDt, fields[headerIndex['end_datetime']], fields[headerIndex['closed_datetime']], fields[headerIndex['resolved_datetime']]);
            incidents.push({
                id,
    ...
    logger_1.logger.info(`✅ Seeded ${inserted} traffic incidents.`);
