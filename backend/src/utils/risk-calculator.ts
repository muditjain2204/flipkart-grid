...
import { EVENT_RISK_MULTIPLIERS } from './heuristics';
...
export function calculateRisk(
  ...
  const typeMultiplier = EVENT_RISK_MULTIPLIERS[eventType];
 ...
 */
function getTimeFactor(startTime: Date): number {
 ...
 */
export function getDensityLevel(vehiclesPerFrame: number): string {
  if (vehiclesPerFrame > 50) return 'CRITICAL';
  if (vehiclesPerFrame > 30) return 'HIGH';
  if (vehiclesPerFrame > 15) return 'MODERATE';
  return 'LOW';
}
